...
export function createLifecycleResetDeleteDispatcher(config = {}) {
  ...
  async function dispatchResetDelete({
    ...
    // `readiness` is validated for signature parity with refresh/restore, but a
    // reset/delete tombstones the live slot: there is no successor envelope to
    // carry a readiness attestation, so it is intentionally not embedded in the
    // request (issue #184 parity note).
