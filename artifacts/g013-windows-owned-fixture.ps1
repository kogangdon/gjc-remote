...
$Head='17afa1ae39d41cd3e143ee976d3c6b5253e0ad44'
$Repo=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$Root=if($env:RUNNER_TEMP){Join-Path $env:RUNNER_TEMP 'gjc13w'}else{'D:\dev\gjc13w'}
$WorkRepo=Join-Path $Root 'repo'
$Output=Join-Path $Root 'output'
$Runner=Join-Path $Root 'g013-owned-runner.mjs'
$Config=Join-Path $Root 'config.json'
$Managed='C:\ProgramData\gjc-remote'
$Workspace='C:\Users\Public\gjc13w-workspace'
$InventoryBase=Join-Path $Managed 'native'
$ReaderBase=Join-Path $Managed 'native-reader'
$Receipt=Join-Path $Repo 'artifacts\g013-windows-owned-receipt.json'
$CleanupProof=Join-Path $Repo 'artifacts\g013-windows-owned-cleanup.json'
$FailurePath=Join-Path $Repo 'artifacts\g013-windows-owned-failure.log'
$Names=@{M='gjc13w_m';B='gjc13w_b';R='gjc13w_r';D='gjc13w_d'}
...
try{
  ...
  Copy-Item (Join-Path $Repo 'artifacts\g013-owned-runner.mjs') $Runner
  ...
  Invoke-As $credentials.D @("`"$Runner`"","`"$Config`"",'read',"`"$(Join-Path $Output 'accepted1.json')`"") 'accepted1'|Out-Null
  Invoke-As $credentials.M @("`"$Runner`"","`"$Config`"",'publish-allow-error',"`"$(Join-Path $Output 'replacement.json')`"","`"$pending`"") 'replacement'|Out-Null
  Invoke-As $credentials.D @("`"$Runner`"","`"$Config`"",'read',"`"$(Join-Path $Output 'accepted2.json')`"") 'accepted2'|Out-Null
  ...
  $holder.WaitForExit()
  $holderReceipt=Read-Json 'holder.json'
  ...
  $evidence=[ordered]@{schemaVersion=1;status='passed';head=$Head;platform='Windows 11 Pro x64 NTFS';principals=[ordered]@{M=$sids.M;B=$sids.B;R=$sids.R;D=$sids.D;SYSTEM='S-1-5-18'};genesis=(Read-Json 'genesis.json');unchanged=(Read-Json 'unchanged.json');pending=(Read-Json 'pending.json');acceptedGenesis=(Read-Json 'accepted1.json');replacement=(Read-Json 'replacement.json');acceptedReplacement=(Read-Json 'accepted2.json');dFenceBlockedMs=$clock.ElapsedMilliseconds;blockedUnchanged=(Read-Json 'blocked.json');holder=(Read-Json 'holder.json');markerFailure=(Read-Json 'marker.json');inventoryFiles=(Read-Json 'inventory-list.json').files;readerFiles=(Read-Json 'reader-list.json').files}
  ...
  if($evidence.acceptedGenesis.status-ne'present'-or$evidence.acceptedGenesis.proof.inventoryGeneration-ne 1-or$evidence.acceptedGenesis.proof.source-ne'native'){throw 'genesis acceptance receipt mismatch'}
  if($evidence.acceptedReplacement.status-ne'present'-or$evidence.acceptedReplacement.proof.inventoryGeneration-ne 2-or$evidence.acceptedReplacement.proof.source-ne'native'){throw 'replacement acceptance receipt mismatch'}
  ...
  $users=@(Get-LocalUser|Where-Object Name -like 'gjc13w_*')
