...
## Architecture (SDK 0.16.4; evidence-scoped integration)

Upgrade status is **BLOCK**, not release-ready. The real SDK oracle reproduces
`SDK_0_16_4_LATE_FOLLOW_UP_NOT_AUTO_CONTINUED`: public `waitForIdle()` resolves
with the exact executable follow-up still queued and no successor. Independent
review approves adapter ownership safety only. Keep the reproducer; do not
substitute remote FIFO delivery or force lower-level continuation.

The topology below remains the implemented integration. Current scoped isolation
evidence is recorded in `docs/verification/issue62-evidence.md`. Adapter
regressions, the real-SDK contract oracle, and local smoke are separate candidate
checks, not tenant-isolation or Broker/Router proof. Older results later in this
file remain explicitly historical.
...
those semantics. The Broker/Router surfaces in SDK 0.16.4 are reserved for a
...
- **Historical `gjc session` / `gjc team` (tmux-backed)** — separately confirmed
  ...
  retires `team`; the current bundled skill is `autoresearch`, and this
  historical Windows observation does not assess the current Broker/Router.
...
The daemon pins `@gajae-code/coding-agent` 0.16.4, requires Bun 1.4.0 or newer,
...
therefore share **one JS event loop**. The 0.16.4 upgrade does not add an
external-session adapter. The focused Windows A/B and B/A probes establish
scope-local settings, active/available model resolution, and registered-cwd
capability lookup; they do not establish CPU parallelism or tenant isolation.
...
0.12.21 SDK sessions coexisting in one process; it is not a 0.16.4 isolation or
concurrency result.
...
- **Scoped versus unscoped SDK state.** SDK 0.16.4 owns independent
  `Settings.loadForScope` instances when settings are omitted. Both creation
  orders preserve scoped policies and available model resolution. Unregistered
  cwd capability fallback and global introspection remain `LAST_CREATED`;
  unfiltered model catalogs are not availability/authorization decisions.
  Do not use these unscoped APIs as per-workDir policy. Empty-fixture auth
...
`bun.lock` (0.16.4). That pin is a runtime dependency fact; it is not isolation,
...
not establish the current 0.16.4 package's ACP conformance, behavior, or
support contract.
...
Broker/Router surfaces in SDK 0.16.4 may be assessed as a future
...
1. **Historical Discord E2E: first manual canary passed 2026-07-28; formal
   evidence table and 0.16.4 rerun still pending.** This predates the 0.16.4
   upgrade and is not current-SDK evidence. The non-Discord relay path is
   designed to be covered by
...
4. **Historical Linux run retained; 0.16.4 platform verification and the macOS
   ...
   dated result is not evidence for SDK 0.16.4 or the Bun 1.4.0 minimum.
...
6. **`/hosts` and progress-streaming (tool-call preview during a running
   ...
   unit-test coverage, but no 0.16.4 adapter result is recorded here yet.
...
when the issue was found; they do not establish how 0.16.4 scopes settings.
...
Regenerate this reasoning with a probe before trusting 0.16.4 scoping
assumptions.
...
isolation nor state bleed for 0.16.4. A focused divergent-session probe must
establish the current behavior before this caveat can be removed. Upstream fix
reference:
