...
const requestKeys = ["version", "kind", "requestId", "genesisTxId", "generation", "readerInstanceId", "readerStartNonce", "routeFingerprint", "nonce", "expiresAt", "requestFingerprint"];
export function validateAdmissionRequest(request) {
  ...
  if (!Number.isSafeInteger(request.generation) || request.generation < 1 || !Number.isSafeInteger(request.expiresAt) || request.expiresAt < 0 || !isHex64(request.requestFingerprint)) fail("request generation, expiry, or hash");
  if (recordHash(request, "requestFingerprint") !== request.requestFingerprint) fail("request fingerprint");
  return request;
}

const grantKeys = ["version", "kind", "grantId", "requestFingerprint", "genesisTxId", "generation", "readerInstanceId", "readerStartNonce", "routeFingerprint", "nonce", "expiresAt", "grantFingerprint"];
export function validateAdmissionGrant(grant, request, now = null, consumedGrantIds = new Set()) {
  ...
  if (!isOpaqueIdentity(grant.grantId) || !Number.isSafeInteger(grant.generation) || !Number.isSafeInteger(grant.expiresAt)) fail("grant id, generation, or expiry");
  for (const key of ["requestFingerprint", "genesisTxId", "generation", "readerInstanceId", "readerStartNonce", "routeFingerprint", "nonce", "expiresAt"]) if (grant[key] !== request[key]) fail("grant does not bind request");
...
const ackKeys = ["version", "kind", "grantFingerprint", "grantId", "genesisTxId", "generation", "readerInstanceId", "readerStartNonce", "routeFingerprint", "readerProjectionFingerprint", "nonce", "ackFingerprint"];
export function validateAdmissionAckRecord(ack) {
  ...
  if (!Number.isSafeInteger(ack.generation) || ack.generation < 1 || recordHash(ack, "ackFingerprint") !== ack.ackFingerprint) fail("ack fingerprint");
  return ack;
}
...
export function validateAdmissionAck(ack, grant, projectionFingerprint) {
  ...
  for (const key of ["grantFingerprint", "grantId", "genesisTxId", "generation", "readerInstanceId", "readerStartNonce", "routeFingerprint", "nonce"]) if (ack[key] !== grant[key]) fail("ack does not bind grant");
...
const proofKeys = ["version", "kind", "genesisTxId", "generation", "zFinalityFingerprint", "readerProjectionFingerprint", "ackFingerprint", "routeFingerprint", "finalityProofFingerprint"];
export function validateFinalityProof(proof, request, zf, ack = null, projectionFingerprint = null) {
  ...
  if (proof.genesisTxId !== request.genesisTxId || proof.generation !== request.generation || proof.zFinalityFingerprint !== zf.zFinalityFingerprint || !isHex64(proof.zFinalityFingerprint) || !isOpaqueIdentity(proof.routeFingerprint) || !isHex64(proof.finalityProofFingerprint)) fail("finality proof relation");
...
export function buildAdmissionGrant(request, { grantId, expiresAt }) {
  validateAdmissionRequest(request);
  const grant = { version: 1, kind: "admission-grant", grantId, requestFingerprint: request.requestFingerprint, genesisTxId: request.genesisTxId, generation: request.generation, readerInstanceId: request.readerInstanceId, readerStartNonce: request.readerStartNonce, routeFingerprint: request.routeFingerprint, nonce: request.nonce, expiresAt, grantFingerprint: null };
...
export function buildAdmissionAck(grant, readerProjectionFingerprint) {
  const ack = { version: 1, kind: "admission-ack", grantFingerprint: grant.grantFingerprint, grantId: grant.grantId, genesisTxId: grant.genesisTxId, generation: grant.generation, readerInstanceId: grant.readerInstanceId, readerStartNonce: grant.readerStartNonce, routeFingerprint: grant.routeFingerprint, readerProjectionFingerprint, nonce: grant.nonce, ackFingerprint: null };
