...
const transactionKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "baselineFingerprint", "transactionFingerprint"];
const uKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "baselineFingerprint", "anchorFingerprint", "targetState", "attestationFingerprint", "authorityReservationFingerprint", "authorityCommitSnapshotFingerprint", "fenceBindingFingerprint", "leaseBindingFingerprint", "readerProjectionFingerprint", "readerInstanceId", "readerStartNonce", "readerVersion", "publication-uFingerprint"];
const pKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "uFingerprint", "stateFingerprint", "targetState", "authorityCommitSnapshotFingerprint", "fenceBindingFingerprint", "leaseBindingFingerprint", "readerInstanceId", "readerStartNonce", "readerVersion", "publication-pFingerprint"];
const sKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "pFingerprint", "stateFingerprint", "payloadFingerprint", "targetState", "authorityCommitSnapshotFingerprint", "fenceBindingFingerprint", "readerVersion", "publication-sFingerprint"];
const cKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "sFingerprint", "stateFingerprint", "payloadFingerprint", "snapshotFingerprint", "authorityCommitSnapshotFingerprint", "fenceBindingFingerprint", "readerInstanceId", "readerStartNonce", "readerVersion", "publication-cFingerprint"];
const qKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "cFingerprint", "baselineFingerprint", "stateFingerprint", "payloadFingerprint", "snapshotFingerprint", "authorityCommitSnapshotFingerprint", "fenceBindingFingerprint", "publication-qFingerprint"];
const zpKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "qFingerprint", "publicationFingerprint", "stateFingerprint", "payloadFingerprint", "snapshotFingerprint", "publication-zpFingerprint"];
const kKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "zpFingerprint", "publicationFingerprint", "authorityCommitSnapshotFingerprint", "checkpointFingerprint", "publication-kFingerprint"];
const yKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "kFingerprint", "publicationFingerprint", "targetState", "authorityCommitSnapshotFingerprint", "fenceBindingFingerprint", "targetFingerprint", "publication-yFingerprint"];
const phaseKeys = ["version", "kind", "txId", "genesisTxId", "fenceGeneration", "generation", "publicationFingerprint", "phase", "publicationStateFingerprint"];
...
const common = (record, kind, keys, field) => {
  if (!exact(record, keys) || record.version !== 1 || record.kind !== kind || !id(record.txId) || !id(record.genesisTxId) || !positiveFence(record.fenceGeneration) || !Number.isSafeInteger(record.generation) || record.generation < 1 || !isHex64(record[field]) || hash(record, field) !== record[field]) fail(`${kind} schema`);
};
...
export function validatePublicationP(record, u, stateFingerprint = undefined) {
  ...
  if (u && typeof u !== "string" && record.fenceGeneration !== u.fenceGeneration) fail("p fence relation");
  return record;
}
export function validatePublicationS(record, p, { stateFingerprint, payloadFingerprint } = {}) {
  ...
  if (p && typeof p !== "string" && record.fenceGeneration !== p.fenceGeneration) fail("s fence relation");
  return record;
}
export function validatePublicationC(record, s, { stateFingerprint, payloadFingerprint, snapshotFingerprint } = {}) {
  ...
  if (s && typeof s !== "string" && record.fenceGeneration !== s.fenceGeneration) fail("c fence relation");
  return record;
}
...
export function validatePublicationZp(record, q, { stateFingerprint, payloadFingerprint, snapshotFingerprint, publicationFingerprint } = {}) {
  ...
  if (q && typeof q !== "string" && record.fenceGeneration !== q.fenceGeneration) fail("zp fence relation");
...
export function validatePublicationK(record, zp, { publicationFingerprint, checkpointFingerprint } = {}) {
  ...
  if (zp && typeof zp !== "string" && record.fenceGeneration !== zp.fenceGeneration) fail("k fence relation");
...
export function validatePublicationY(record, k, targetFingerprint = undefined, publicationFingerprint = undefined) {
  ...
  if (k && typeof k !== "string" && record.fenceGeneration !== k.fenceGeneration) fail("y fence relation");
...
export function validatePublicationState(record, transaction) {
  ...
  if (transaction && (record.txId !== transaction.txId || record.genesisTxId !== transaction.genesisTxId || record.generation !== transaction.generation || record.fenceGeneration !== transaction.fenceGeneration)) fail("publication state transaction relation"); return record;
}
