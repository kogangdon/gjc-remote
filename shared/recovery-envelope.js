...
const txKeys = ["version", "kind", "txId", "anchorFingerprint", "fenceGeneration", "operation", "expectedRevision", "expectedFingerprint", "oldSnapshot", "candidateSnapshot", "oldMappingId", "oldMappingGeneration", "candidateMappingId", "candidateMappingGeneration", "immutableGeneration", "txFingerprint"];
const pubKeys = ["version", "kind", "txId", "anchorFingerprint", "fenceGeneration", "phase", "candidateSnapshotFingerprint", "targetSnapshotFingerprint", "targetIdentityFingerprint", "targetAclFingerprint", "publicationFingerprint"];
const backupKeys = ["version", "kind", "txId", "anchorFingerprint", "fenceGeneration", "phase", "oldSnapshotFingerprint", "backupIdentityFingerprint", "backupAclFingerprint", "backupFingerprint"];
const rcKeys = ["version", "kind", "txId", "anchorFingerprint", "fenceGeneration", "phase", "txFingerprint", "publicationFingerprint", "backupFingerprint", "recoveryFingerprint"];
...
export function validateMappingRecoveryTransaction(tx) {
  ...
  if (tx.oldSnapshot.configFingerprint === tx.candidateSnapshot.configFingerprint ||
      tx.oldSnapshot.fenceGeneration > tx.fenceGeneration ||
      tx.candidateSnapshot.fenceGeneration !== tx.fenceGeneration ||
      recordHash(tx, "txFingerprint") !== tx.txFingerprint) fail("mapping transaction fingerprint");
  return tx;
}

export function validateMappingRecoveryPublication(pub, tx) {
  if (!exact(pub, pubKeys) || pub.version !== 1 || pub.kind !== "mapping-recovery-pub" || !positiveFence(pub.fenceGeneration) || !phase.has(pub.phase) || !isOpaqueIdentity(pub.txId) || !isHex64(pub.anchorFingerprint) || !isHex64(pub.candidateSnapshotFingerprint) || !nullableHex(pub.targetSnapshotFingerprint) || !nullableHex(pub.targetIdentityFingerprint) || !nullableHex(pub.targetAclFingerprint) || !isHex64(pub.publicationFingerprint)) fail("mapping publication schema");
  validateMappingRecoveryTransaction(tx);
  if (pub.txId !== tx.txId || pub.anchorFingerprint !== tx.anchorFingerprint || pub.fenceGeneration !== tx.fenceGeneration || pub.candidateSnapshotFingerprint !== tx.candidateSnapshot.configFingerprint || (pub.phase === "prepared" ? [pub.targetSnapshotFingerprint, pub.targetIdentityFingerprint, pub.targetAclFingerprint].some((value) => value !== null) : [pub.targetSnapshotFingerprint, pub.targetIdentityFingerprint, pub.targetAclFingerprint].some((value) => value === null)) || recordHash(pub, "publicationFingerprint") !== pub.publicationFingerprint) fail("mapping publication relation");
  return pub;
}

export function validateMappingRecoveryBackup(backup, tx) {
  if (!exact(backup, backupKeys) || backup.version !== 1 || backup.kind !== "mapping-recovery-bk" || !positiveFence(backup.fenceGeneration) || !phase.has(backup.phase) || !isOpaqueIdentity(backup.txId) || !isHex64(backup.anchorFingerprint) || !isHex64(backup.oldSnapshotFingerprint) || !nullableHex(backup.backupIdentityFingerprint) || !nullableHex(backup.backupAclFingerprint) || !isHex64(backup.backupFingerprint)) fail("mapping backup schema");
  validateMappingRecoveryTransaction(tx);
  if (backup.txId !== tx.txId || backup.anchorFingerprint !== tx.anchorFingerprint || backup.fenceGeneration !== tx.fenceGeneration || backup.oldSnapshotFingerprint !== tx.oldSnapshot.configFingerprint || [backup.backupIdentityFingerprint, backup.backupAclFingerprint].some((value) => value === null) || recordHash(backup, "backupFingerprint") !== backup.backupFingerprint) fail("mapping backup relation");
  return backup;
}

export function validateMappingRecoveryCheckpoint(rc, tx, pub, backup) {
  if (!exact(rc, rcKeys) || rc.version !== 1 || rc.kind !== "mapping-recovery-rc" || !positiveFence(rc.fenceGeneration) || !phase.has(rc.phase) || !isOpaqueIdentity(rc.txId) || !isHex64(rc.anchorFingerprint) || !isHex64(rc.txFingerprint) || !isHex64(rc.publicationFingerprint) || !isHex64(rc.backupFingerprint) || !isHex64(rc.recoveryFingerprint)) fail("mapping recovery checkpoint schema");
  validateMappingRecoveryPublication(pub, tx); validateMappingRecoveryBackup(backup, tx);
  if (rc.txId !== tx.txId || rc.anchorFingerprint !== tx.anchorFingerprint || rc.fenceGeneration !== tx.fenceGeneration || rc.phase !== pub.phase || rc.phase !== backup.phase || rc.txFingerprint !== tx.txFingerprint || rc.publicationFingerprint !== pub.publicationFingerprint || rc.backupFingerprint !== backup.backupFingerprint || recordHash(rc, "recoveryFingerprint") !== rc.recoveryFingerprint) fail("mapping recovery checkpoint relation");
  return rc;
}
...
export function buildMappingRecoveryRecords({ tx, phase: currentPhase, targetSnapshotFingerprint = null, targetIdentityFingerprint = null, targetAclFingerprint = null, backupIdentityFingerprint = null, backupAclFingerprint = null }) {
  validateMappingRecoveryTransaction(tx);
  const publication = { version: 1, kind: "mapping-recovery-pub", txId: tx.txId, anchorFingerprint: tx.anchorFingerprint, fenceGeneration: tx.fenceGeneration, phase: currentPhase, candidateSnapshotFingerprint: tx.candidateSnapshot.configFingerprint, targetSnapshotFingerprint, targetIdentityFingerprint, targetAclFingerprint, publicationFingerprint: null };
  publication.publicationFingerprint = recordHash(publication, "publicationFingerprint");
  const backup = { version: 1, kind: "mapping-recovery-bk", txId: tx.txId, anchorFingerprint: tx.anchorFingerprint, fenceGeneration: tx.fenceGeneration, phase: currentPhase, oldSnapshotFingerprint: tx.oldSnapshot.configFingerprint, backupIdentityFingerprint, backupAclFingerprint, backupFingerprint: null };
  backup.backupFingerprint = recordHash(backup, "backupFingerprint");
  const checkpoint = { version: 1, kind: "mapping-recovery-rc", txId: tx.txId, anchorFingerprint: tx.anchorFingerprint, fenceGeneration: tx.fenceGeneration, phase: currentPhase, txFingerprint: tx.txFingerprint, publicationFingerprint: publication.publicationFingerprint, backupFingerprint: backup.backupFingerprint, recoveryFingerprint: null };
...
const suffixKeys = ["version", "kind", "txId", "fenceGeneration", "requestFingerprint", "finalityFingerprint", "receiptFingerprint", "admissionOpen", "phase", "suffixFingerprint"];
export function validateGenesisSuffixRecovery(value, recovery) { if (!exact(value, suffixKeys) || value.version !== 1 || value.kind !== "genesis-suffix-recovery" || value.phase !== "terminal" || !isOpaqueIdentity(value.txId) || !positiveFence(value.fenceGeneration) || !isHex64(value.requestFingerprint) || !isHex64(value.finalityFingerprint) || !isHex64(value.receiptFingerprint) || typeof value.admissionOpen !== "boolean" || !isHex64(value.suffixFingerprint) || recordHash(value, "suffixFingerprint") !== value.suffixFingerprint) fail("genesis suffix schema"); if (!recovery || value.txId !== recovery.txId || value.fenceGeneration !== 1 || value.requestFingerprint !== recovery.requestFingerprint) fail("genesis suffix recovery relation"); return value; }
